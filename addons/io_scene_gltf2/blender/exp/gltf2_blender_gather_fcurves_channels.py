...
def __needs_baking(obj_uuid: str,
    ...
    if export_settings['vtree'].nodes[obj_uuid].blender_object.type == "ARMATURE":
        animation_target = gltf2_blender_get.get_object_from_datapath(export_settings['vtree'].nodes[obj_uuid].blender_object, [c for c in channels if c is not None][0].data_path)
