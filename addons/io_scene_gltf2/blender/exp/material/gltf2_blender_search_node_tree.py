    ...
    if node.node is not None:
        ...
        if kind == 'RGB':
            if node.node.type == 'MIX' and node.node.data_type == "RGBA" and node.node.blend_type == 'MULTIPLY':
                ...
                x1, path_x1 = get_const_from_socket(NodeSocket(node.node.inputs[6], node.group_path), kind)
                x2, path_x2 = get_const_from_socket(NodeSocket(node.node.inputs[7], node.group_path), kind)
        if kind == 'VALUE':
            if node.node.type == 'MATH' and node.node.operation == 'MULTIPLY':
                x1, path_x1 = get_const_from_socket(NodeSocket(node.node.inputs[0], node.group_path), kind)
                x2, path_x2 = get_const_from_socket(NodeSocket(node.node.inputs[1], node.group_path), kind)
...
def get_const_from_socket(socket, kind):
    if not socket.socket.is_linked:
        if kind == 'RGB':
            if socket.socket.type != 'RGBA': return None, None
            return list(socket.socket.default_value)[:3], "node_tree." + socket.socket.path_from_id() + ".default_value"
        if kind == 'VALUE':
            if socket.socket.type != 'VALUE': return None, None
            return socket.socket.default_value, "node_tree." + socket.socket.path_from_id() + ".default_value"
    ...
    if prev_node.node is not None:
        if kind == 'RGB' and prev_node.node.type == 'RGB':
            return list(prev_node.node.outputs[0].default_value)[:3], "node_tree." + prev_node.node.outputs[0].path_from_id() + ".default_value"
        if kind == 'VALUE' and prev_node.node.type == 'VALUE':
            return prev_node.node.outputs[0].default_value, "node_tree." + prev_node.node.outputs[0].path_from_id() + ".default_value"
