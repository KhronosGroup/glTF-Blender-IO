...
def export_volume(blender_material, export_settings):
    ...
    if len(thickness_slots) > 0:
        combined_texture, use_active_uvmap, uvmap_attribute_name, _ = gltf2_blender_gather_texture_info.gather_texture_info(
            thicknesss_socket,
            thickness_slots,
            (),
            export_settings,
        ...
        if uvmap_attribute_name:
            uvmap_attribute_names.update({"thicknessTexture": uvmap_attribute_name})

    return Extension('KHR_materials_volume', volume_extension, False), use_actives_uvmaps, uvmap_attribute_names
