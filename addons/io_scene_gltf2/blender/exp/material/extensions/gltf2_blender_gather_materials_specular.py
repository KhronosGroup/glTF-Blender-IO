...
def export_original_specular(blender_material, export_settings):
    ...
    if specular_non_linked is True:
        ...
        if gltf2_blender_get.has_image_node_from_socket(original_specular_socket):
            original_specular_texture, original_specular_use_active_uvmap, uvmap_attribute_name_original_specular, _ = gltf2_blender_gather_texture_info.gather_texture_info(
            ...
            if uvmap_attribute_name_original_specular:
                uvmap_attrribute_names.update({"specularTexture":uvmap_attribute_name_original_specular})
    ...
    if specularcolor_non_linked is True:
        ...
        if gltf2_blender_get.has_image_node_from_socket(original_specularcolor_socket):
            original_specularcolor_texture, original_specularcolor_use_active_uvmap, uvmap_attribute_name_specular, _ = gltf2_blender_gather_texture_info.gather_texture_info(
            ...
            if uvmap_attribute_name_specular:
                uvmap_attrribute_names.update({"specularColorTexture": uvmap_attribute_name_specular})

    return Extension('KHR_materials_specular', specular_extension, False), use_actives_uvmaps, uvmap_attrribute_names
...
def export_specular(blender_material, export_settings):
    ...
    if no_texture:
        ...
        specularColorTexture, use_active_uvmap, uvmap_attribute_name_specular, specularColorFactor = gltf2_blender_gather_texture_info.gather_texture_info(
            primary_socket,
            sockets,
            export_settings,
            filter_type='ANY')
        ...
        if use_active_uvmap:
            use_actives_uvmaps.append("specularColorTexture")
        if uvmap_attribute_name_specular:
            uvmap_attribute_names.update({"specularColorTexture": uvmap_attribute_name_specular})
    ...
    return specular_extension, use_actives_uvmaps, uvmap_attribute_names
