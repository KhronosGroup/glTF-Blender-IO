...
##################################################################################################
    ...
    # # Normalize node
    # node = mh.node_tree.nodes.new('ShaderNodeVectorMath')
    # node.operation = 'NORMALIZE'
    # node.location = x - 180, y
    # mh.node_tree.links.new(anisotropy_tangent_socket, node.outputs[0])
    # anisotropy_tangent_socket = node.inputs[0]
    ...
    # # Combine XYZ
    # combine_node = mh.node_tree.nodes.new('ShaderNodeCombineXYZ')
    # combine_node.location = x - 180*2, y
    # mh.node_tree.links.new(anisotropy_tangent_socket, combine_node.outputs[0])

    # # Separate XYZ
    # sep_node = mh.node_tree.nodes.new('ShaderNodeSeparateColor')
    # sep_node.location = x - 180*3, y
    # mh.node_tree.links.new(combine_node.inputs[0], sep_node.outputs[0])
    # mh.node_tree.links.new(combine_node.inputs[1], sep_node.outputs[1])
    ...
    #     location=(x - 180*4, y),
    #     is_data=True,
    #     color_socket=sep_node.inputs[0]
    # )
